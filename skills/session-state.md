...
이 파일은 스킬들이 공유하는 세션 상태의 필드 정의와 소유권을 기술한다.
...
mlcc-lot-validation
  ...
  └─ writes → lot_id, chip_prod_id

mlcc-optimal-design-doe        mlcc-convergence-search
  ├─ reads  ← lot_id              ├─ reads  ← lot_id
  └─ writes → targets, params,    └─ writes → targets, params,
...
## Phase 4 — 최종 설계값 (design-dispatch 입력)
