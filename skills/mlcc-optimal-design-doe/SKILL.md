...
각 패턴을 실행할 때 해당 reference를 반드시 참고한다. reference에는 정확한 tool 호출 예시, 파라미터 조합, 대화 예시 등 실행에 필수적인 상세 정보가 있다.

- `references/tool-contracts.md`: tool의 정확한 입출력 계약. **첫 tool 호출 전에 반드시 참고한다.**
- `references/pattern-ref-lot-selection.md`: REF LOT 선정 — 필터 파라미터 매핑, 결과 없음 대응
- `references/pattern-validation.md`: LOT 검증 — 버전별 판정, 부족인자 보충 흐름
- `references/pattern-optimal.md`: 최적설계 DOE — params 리스트 생성법, rerun 규칙. **최적설계 실행 전 반드시 참고한다.**
- `references/pattern-reliability.md`: 신뢰성 시뮬레이션 — halt 조건 확인 절차, 설계값 경로. **신뢰성 실행 전 반드시 참고한다.**
- `references/pattern-autonomous.md`: 자율 반복 — 4가지 패턴 조합(A~D). **자율 반복 실행 전 반드시 참고한다.**
- `references/pattern-convergence.md`: 수렴 탐색 — 4-Phase 방법론, 파라미터 조정 룰. **수렴 탐색 실행 전 반드시 참고한다.**
- `references/prompt-examples.md`: 한국어 사용자 질의와 응답 패턴 예시

## 핵심 원칙

- `lot_id`가 없고 인접기종 chip_prod_id 목록이 있으면, `find_ref_lot_candidate`로 REF LOT를 먼저 선정한다. 사용자가 품질 필터를 지정하면 해당 파라미터를 적용한다. 상세 가이드는 `references/pattern-ref-lot-selection.md` 참고.
- `lot_id`가 없으면 가장 먼저 요청한다.
- 새 `lot_id`가 들어오면 반드시 `get_first_lot_detail`로 설계정보를 state에 로드한 뒤, `check_optimal_design`을 호출한다.
- `check_optimal_design` 결과에서 **`fully_satisfied_versions`가 한 개라도 있으면 해당 버전으로 시뮬레이션을 바로 진행한다.** 모든 버전이 충족될 때까지 기다리지 않는다.
- `부족인자`가 있는 버전은 사용자에게 참고 정보로 안내하고, 원하면 `update_lot_reference`로 채워 추가 버전도 활성화할 수 있다.
...
- `halt_voltage`와 `halt_temperature`는 **사용자에게 반드시 한 번 확인한다.** 기본값(5)을 그대로 쓰지 않는다.
