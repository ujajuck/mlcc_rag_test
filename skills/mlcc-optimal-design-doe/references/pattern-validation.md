...
```json
{
  "충족인자": {"Sheet 두께": 4.8, "전극 폭": 670, "전극 수": 158, "Margin L": 80},
  "부족인자": ["유전체 두께", "Cover 두께", "유전상수", "Margin W"],
  "ref_values": {"Sheet 두께": 4.8, "전극 폭": 670, ..., "유전체 두께": null, ...}
}
```
...
update_lot_reference(
  lot_id="L240215-B",
  factors={"유전체 두께": 3.2, "Cover 두께": 28, "유전상수": 2600, "Margin W": 55}
)
```
...
사용자가 일부만 제공해도 된다. 예: "유전체 두께 3.2, Cover 두께 28만 먼저 넣어줘"
→ 나머지(유전상수, Margin W)는 다음 턴에 받으면 된다.

## 충족인자 값 활용

충족인자에 반환된 값은 이후 params 기본값 제안에 활용한다.
예: `check_optimal_design`이 `Sheet 두께: 4.8`을 반환했으면, params 수집 시 "Sheet T의 ref lot 값은 4.8um입니다. 이 값 기준으로 범위를 잡을까요?" 같은 제안이 가능하다.
