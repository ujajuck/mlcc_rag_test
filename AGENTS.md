...
*문서 유지 규칙*
- 시스템 흐름 기준 문서는 `SYSTEM_FLOW.md` 이고, 흐름 다이어그램 전용 문서는 루트의 `mermaid.md` 이다.
- `CLAUDE.md` 는 만들지 말고, 문서 유지 규칙은 이 `AGENTS.md` 파일에 통일한다.
- 아래 항목이 바뀌면 `SYSTEM_FLOW.md` 와 `mermaid.md`를 반드시 함께 확인하고, 필요한 경우 같이 수정한다.
...
- 문서 갱신 원칙은 아래와 같다.
  ...
  - `mermaid.md` 는 mermaid 코드블록만 두는 다이어그램 전용 문서로 유지한다.
